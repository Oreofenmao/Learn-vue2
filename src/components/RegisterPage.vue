<template>
  <div class = "loginRegister">
    ...
    <form @submit.prevent="submitForm">
        ...
        <button type="button" @click="switchPage"> 前往登录页面 </button>
...
export default {
    ...
    methods:{
      ...
      // 发送 POST 请求到 Django 后端的 /register/ 路由
        this.$axios.post('/register/', formData)
            .then(response => {
                console.log('注册成功', response);  // 注册成功后的操作（如跳转、提示等）
            })
            .catch(error => {
                console.log('注册失败', error.response.data.message);  // 注册失败的错误处理
            });
        }
    }
}
